...
tasks.register("generateBaselineProfileFromDevice") {
    ...
    doLast {
        ...
        exec { commandLine(adb, "shell", "cmd", "package", "compile", "-m", "speed-profile", "-f", packageName) }
        exec { commandLine(adb, "shell", "am", "start", "-W", "$packageName/.MainActivity") }
        Thread.sleep(5_000)
        val profile = ByteArrayOutputStream()
        exec {
            commandLine(adb, "shell", "cmd", "package", "dump-profiles", packageName)
            standardOutput = profile
