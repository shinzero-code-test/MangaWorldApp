            ...
            {data.dailyActive.map((d, i) => {
              ...
              return (
                <div key={`${d.date}-${i}`} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-xs text-[var(--muted-foreground)]">{formatAr(d.users)}</span>
                  <div
            ...
            <div className="space-y-3">
              {data.topManga.map((m, i) => (
                <div key={`${m.name}-${i}`} className="flex items-center gap-3">
                  <span className="text-sm font-bold text-[var(--muted-foreground)] w-5">{formatAr(i + 1)}</span>
                  <div className="flex-1">
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span>{m.name}</span>
                      <span className="text-[var(--muted-foreground)]">{formatAr(m.reads)}</span>
                    </div>
